pacman::p_load(data.table, dplyr, ggplot2, fst, ggthemes, extrafont)
...
p = d %>%
  ...
  labs(x = "cumulative driving time (hours)",
       ...
       The grey bands shows 95% confidence intervals using normal approximation.") +
  scale_x_continuous(breaks = 0:11) + ggthemes::geom_rangeframe() +
  ggthemes::theme_tufte(base_family = "LMRoman") + #'GillSans'
  theme(axis.text = element_text(size = 9, family = "LMRoman"),
        plot.caption = element_text(size = 10, family = "LMRoman"),
        axis.title = element_text(size = 12, family = "LMRoman"))


ggsave("figures/p_11hours.png", width = 10, height = 6.18, dpi = 600)
ggsave("figures/p_11hours.eps", width = 10, height = 6.18)
ggsave("figures/p_11hours.pdf", p, width = 10, height = 6.18, device = cairo_pdf)
