...
Data
====
The data were collected by a leading freight shipping trucking company (we will name it as Company A for confidentiality reasons) in the United States. From April 2015 to March 2016, trucks in Company A were equipped with in-vehicle data acquisition systems (DAGs) that collect real-time *ping* and *SCEs* data. Details of these two data sources will be introduced in the following subsection. For demonstration purposes, in this study, we selected 496 regional truck drivers who move freights in a region and surrounding states. Apart from these vehicle driving data, demographic variables inclduing age, gender, and race were also provided to the research team. The drivers were anonymized to ensure confidentiality, instead, a unique identification number was provided for each driver to link the three data sources. The study protocol was reviewed and approved by the Institutional Review Board of Saint Louis University.

Ping and SCEs data
------------------
The DAGs ping irregularly (typically every a couple of seconds to minutes) as the truck goes on road. Each ping collects several key variables, including the date and time (year, month, day, hour, minute, and second), latitude and longitude (specific to five decimal places), driver identity number, and speed at that second. In total, 13,187,289 rows of ping data were generated by the 496 truck drivers. 

Apart from ping data, Company A also collected real-time SCEs data for all their trucks. In constrast to irregularly collected ping data, SCEs were recorded whenever pre-determined kinematic thresholds were triggered.
There were 12,458 critical events occurred to these 496 truck drivers during the study period. Four types of critical events were recorded in this critical events data. \hl{The number of SCEs.}
...
\begin{figure}%[htb]
    ...
    \begin{subfigure}[a]{\textwidth}
        \centering
        \includegr-aphics[width=\linewidth]{figures/map_active_ping_Aim2.png}%
    ...
    \end{subfigure}
    \caption{Geographical point patterns of moving and stopped pings generated by the 496 sample drivers.}
