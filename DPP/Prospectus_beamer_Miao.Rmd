...
- \textcolor{slublue}{Subsampling MCMC} such as energy conserving subsampling Hamiltonian Monte Carlo. [@dang2019hamiltonian]
...
1. **Trip**: for each of the truck drivers, if the real-time ping data showed that the truck was \textcolor{slublue}{not moving for more than 20 minutes}, the ping data will be separated into two different trips (~200,000 rows), 
2. **30-minute intervals**: as the length of a trip can vary significantly from 5 minutes to more than 8 hours, I will transform the trips data into \textcolor{slublue}{standardized 30-minute fixed intervals} according to the starting and ending time of trips (~1 million rows),
2. **Shift**: the trips data will be further divided into different shifts if the specific driver was \textcolor{slublue}{not moving for eight hours}.
...
### Aim 2 - Model 1: Bayesian hierarchical logistic regression
...
### Aim 2 - Model 2: Bayesian hierarchical Poisson regression
...
### Aim 2 - Model 3: motivation for recurrent event models
