...
[An attempt to explain the Gibbs sampler](https://jcahoon.netlify.com/gibbs-for-dummies-like-me/)
...
As shown in Equation \@ref(eq:hamiltonian), the Hamiltonian can be decomposed into two components $H(\mathbf{q}, \mathbf{p}) = K(\mathbf{p}, \mathbf{q}) + V(\mathbf{q})$. 
$K(p, q)$ is the *kinetic energy* that is unconstrained and need to be decided during implementation, 
and $V(q)$ is the *potential energy* that only depends on the target posterior distribution (precisely, it is the negative log of the posterior density) [@betancourt2017conceptual; @monnahan2017faster].
