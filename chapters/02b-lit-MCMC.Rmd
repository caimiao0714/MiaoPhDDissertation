...
After decades of successful empirical trials in physics, @hastings1970monte proposed a more generalized form of the Metropolis algorithm, in which the proposal distribution can be arbitrary (Theorem \@ref(thm:MHalgorithm)), but the acceptance probability $\alpha^\star$ is modified as shown in Equation \@ref(eq:MHalpha). This Metropolis-Hasting (MH) algorithm is the most classic and widely-known MCMC algorithm used in multiple fields.

```{theorem, name="The Metropolis-Hasting Algorithm", label="MHalgorithm"}
...
(\#eq:MHalpha)
\end{equation}
5. Set $\theta^{(t+1)} = \theta^{(\text { prop })}$ with the probability of $\alpha^\star$; otherwise set $\theta^{(t+1)} = \theta^{(t)}$ and return to 3 until the desired number of iterations is reached.
...
```{theorem, name="Gibbs Sampler", label="gibbs"}
...
The M-H algorithm can be very inefficient in big data settings as a consequence of sparse high-density parameter space, the cost of evaluating the entire data at each step, and high correlation between parameters. Originally proposed by @duane1987hybrid with the name of Hybrid Monte Carlo, the Hamiltonian Monte Carlo (HMC) changes the random-walk behavior into a deterministic one by adding auxiliary momentum parameters $p_n$, thus more efficiently explores the high-density regions in big data settings compared to the traditional M-H algorithm or the Gibbs sampler [@betancourt2017conceptual; @wang2019hamiltonian]. 

```{theorem, name="Himiltonian mechanics", label="hamiltoninamechanics"}
Let $\mathbf{q}$ denote the position vector and $\mathbf{p}$ denote the momentum vector in the conservative dynamics physics system. The combination $(\mathbf{q}, \mathbf{p})$ then defines a position-momentum phase space. The evolution of this system is then governed by the *Hamiltonian equations*:
\begin{equation}
\begin{aligned}
...
Where $H = H(\mathbf{q}, \mathbf{p})$ is the *Hamiltonian* that is the total energy of the system.
```

The Hamiltonian can be decomposed into two components $H(\mathbf{q}, \mathbf{p}) = K(p, q) + V(q)$. $K(p, q)$ is the kinetic energy that is unconstrained and need to be decided during implementation, and $V(q)$ is the potential energy that only depends on the target posterior distribution [@betancourt2017conceptual].
