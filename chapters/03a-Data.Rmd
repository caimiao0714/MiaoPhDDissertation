...
1. *SCEs*: the SCEs are merged to the two aggregated tables by drivers and of the time of SCEs fall between the start and end time of the aggregated tables,
2. *Drivers*: the age of drivers are merged to the two aggregated tables using driver unique ID,
3. *Road geometry*: the road geometry variables will firstly be merged back to the original ping data by driver ID, latitude, and longitude. Then these road geometry variables will averaged by taking the mean for each 30-minute interval and shift to the aggregated levels,
4. *Weather*: the weather variables will firstly also be merged back to the original ping data by driver ID, latitude, longitude, date, and time. These weather variables will then be averaged by taking the mean for each 30-minute interval and shift to the aggregated levels.

The resulting 30-minute intervals and shifts tables are demonstrated in Table \@ref(tab:analysis30int) and Table \@ref(tab:analysisshift). Table \@ref(tab:analysisshiftevent) demonstrates the SCEs table, with time to events calculated as the time difference in hours between the time of the SCE and the starting time of the corresponding shift.
