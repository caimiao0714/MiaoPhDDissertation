...
\href{https://jcahoon.netlify.com/gibbs-for-dummies-like-me/}{An attempt to explain the Gibbs sampler}
...
As shown in Equation \eqref{eq:hamiltonian}, the Hamiltonian can be decomposed into two components \(H(\mathbf{q}, \mathbf{p}) = K(\mathbf{p}, \mathbf{q}) + V(\mathbf{q})\).
\(K(p, q)\) is the \emph{kinetic energy} that is unconstrained and need to be decided during implementation,
and \(V(q)\) is the \emph{potential energy} that only depends on the target posterior distribution (precisely, it is the negative log of the posterior density) (Betancourt \protect\hyperlink{ref-betancourt2017conceptual}{2017}; Monnahan, Thorson, and Branch \protect\hyperlink{ref-monnahan2017faster}{2017}).
The total energy (the Hamiltonian) remains constant over time.

\href{http://elevanth.org/blog/2017/11/28/build-a-better-markov-chain/}{Markov Chains: Why Walk When You Can Flow?}
