...
Recent ten years witnessed an explosive growth of data size with regard to both the the number of rows and columns. This poses a major challenge to Bayesian methods using MCMC, which requires the algorithm to evaluate the entire data at each step of iteration (Bardenet, Doucet, and Holmes \protect\hyperlink{ref-bardenet2017markov}{2017}). In applied analysis, researchers often need to set thousands of iterations to reach stable posterior distribution, which takes hours or days to implement a single model. On the other hand, when there is high correlation between different parameters that often occur in the case of many parameters, neither the M-H algorithm or Gibbs sampler can efficiently generate samples from the posterior distribtion.

\hypertarget{hamiltonian-monte-carlo}{%
\subsection{Hamiltonian Monte Carlo}\label{hamiltonian-monte-carlo}}

Random walk M-H algorithm can have very low acceptance rates and thus be very inefficient when there is correlation between parameters or the parameter space has high curvature. The Hamiltonian Monte Carlo changes the random-walk behavior into a non-random-walk one by adding auxiliary momentum parameters \(p_n\) (Betancourt \protect\hyperlink{ref-betancourt2017conceptual}{2017}).
...
OpenStreetMap API, which can be accessed from the \texttt{osmar} package in R.
...
Canani, SF, AB John, MG Raymundi, S Schönwald, and SS Menna Barreto. 2005. ``Prevalence of Sleepiness in a Group of Brazilian Lorry Drivers.'' \emph{Public Health} 119 (10). Elsevier: 925--29.
...
Nantulya, Vinand M, and Michael R Reich. 2002. ``The Neglected Epidemic: Road Traffic Injuries in Developing Countries.'' \emph{Bmj} 324 (7346). British Medical Journal Publishing Group: 1139--41.
...
Odero, Wilson, Meleckidzedeck Khayesi, and PM Heda. 2003. ``Road Traffic Injuries in Kenya: Magnitude, Causes and Status of Intervention.'' \emph{Injury Control and Safety Promotion} 10 (1-2). Taylor \& Francis: 53--61.
...
Pérez-Chada, Daniel, Alejandro J Videla, Martin E O'flaherty, Patricia Palermo, Jorgelina Meoni, Maria I Sarchi, Marina Khoury, and Joaquı'n Durán-Cantolla. 2005. ``Sleep Habits and Accident Risk Among Truck Drivers: A Cross-Sectional Study in Argentina.'' \emph{Sleep} 28 (9). Oxford University Press: 1103--8.
...
Zhang, Guangnan, Kelvin KW Yau, Xun Zhang, and Yanyan Li. 2016. ``Traffic Accidents Involving Fatigue Driving and Their Extent of Casualties.'' \emph{Accident Analysis \& Prevention} 87. Elsevier: 34--42.
