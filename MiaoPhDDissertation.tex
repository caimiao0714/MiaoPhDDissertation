...
\vspace{1cm}
\begin{center}
\linespread{2}\normalsize \bfseries \MakeUppercase{Modeling Truck Drivers' Safety Critical Events: Efficient Bayesian Hierarchical Statistical and Reliability Models}
\end{center}
...
Stern et al. (\protect\hyperlink{ref-stern2019data}{2019})
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{include_graphics}\NormalTok{(}\StringTok{"figs/Data_merging.png"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}
\includegraphics[width=44.44in]{figs/Data_merging} \caption{Flow chart of data aggregation and merging}\label{fig:unnamed-chunk-1}
\end{figure}
...
\beta_{0, d} &\sim N(\mu_0, \sigma_0^2), \quad d = 1, 2, \cdots, D\\
\beta_{1, d} &\sim N(\mu_1, \sigma_1^2), \quad d = 1, 2, \cdots, D
...
\label{pois}
\begin{split}
\text{N} & \sim \text{Poisson}(t\cdot\lambda)\\
\lambda_{i, d(i)} & =\exp(\beta_{0, d(i)} + \beta_{1, d(i)} \cdot \text{CT} + \mathbf{\xi} \cdot \mathbf{W} + \mathbf{\nu} \cdot \mathbf{D})
\end{split}
\end{align}
