...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Aim1}: Examine the association between truck crashes and SCEs using a Bayesian Gamma-Poisson regression. I hypothesize that
\item
  \textbf{Aim2}: Construct three scalable Bayesian hierarchical models for SCEs: logistic regression, Poisson regression, and non-homogeneous Poisson process with the power law process intensity function.
\end{enumerate}
...
\tightlist
\item
  \textbf{Aim3}: Propose an innovative reliability model that accounts for both within shift cumulative driving time and between-trip rest time.
\end{enumerate}
